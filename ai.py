    ...
    for classifier in models:
        ...
        if type(classifier).__name__ == "Sequential":
            ...
            visualkeras.layered_view(
                model, to_file=os.path.join("models", "Sequential", "architecture.png")
            ).show()
            ...
            predictions = np.argmax(predictions, axis=1).reshape(
                (predictions.shape[0],)
            )
            ...
            with open(os.path.join("models", "report.txt"), "a") as fh:
                ...
                for i in range(7):
                    try:
                        ...
                        fh.write(
                            "Layer " + str(i) + " config: " + str(layer_config) + "\n"
                        )
                    except:
                        break
                fh.write("\n")
            model_path = os.path.join(
                "models",
                "Sequential",
                "model_" + str(round(metrics[1], 2) * 100) + ".png",
            )
            tf.keras.utils.plot_model(
                model, to_file=model_path, show_shapes=True, show_layer_names=True
            )
            ...
            x_train, x_test, y_train, y_test = train_test_split(
                pixels, np.array(labels_nr), test_size=TEST_SIZE
            )
    ...
    while True:
        while game.game_active:
            ...
            if game.NN:
                ...
                ]  # if np.amax(predictions) > 0.7 else 'noop'
            else:
            ...
            if last_action != action:
                if action == "down" and jump:
                    ...
                    action == "left"
                    and last_action == "right"
                    or action == "right"
                    and last_action == "left"
                ):
                    action = "noop"
                else:
            ...
            print(
                "{} with {:.2f} percent certainty".format(action, 100 * np.max(score))
            )
...
def load_data():
    ...
    if len(sys.argv) > 2:
        ...
        if sys.argv[2] == "load":
            images = np.load(
                os.path.join("dataset_preloaded", "images" + str(IMG_WIDTH) + ".npy")
            )
            labels = np.load(
                os.path.join("dataset_preloaded", "labels" + str(IMG_WIDTH) + ".npy")
            )
            pixels = np.load(
                os.path.join("dataset_preloaded", "pixels" + str(IMG_WIDTH) + ".npy")
            )
        ...
        for folder in folders:
            ...
            for image in os.listdir(folder_path):
                ...
                im = cv2.resize(
                    im, (IMG_WIDTH, IMG_HEIGHT), interpolation=cv2.INTER_AREA
                )
                images.append(im)
        ...
        pixels = x_data.flatten().reshape(
            int(len(images)), int(x_data.size / len(images))
        )
